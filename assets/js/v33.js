...
(function(){
  ...
  document.addEventListener("DOMContentLoaded", ()=>{
    ...
    console.info("[V33] Balanced Nimo global smoke + DreamLab projects OK");
  });
})();
...
/* V33 — Nimo balanced global smoke.
   O motor original do Nimo escuta eventos no window. Não cria overlay.
   O sweep inicial é curto e suave para não estourar a tela. */
(function(){
  "use strict";
  ...
  function initialSoftSweep(){
    ...
    function frame(now){
      ...
      const x = w * (.18 + .56 * t);
      const y = h * (.34 + Math.sin(t * Math.PI * 2.2) * .10);
  ...
  function initDreamLabProjectPreview(){
    ...
    document.querySelectorAll(".project-item").forEach(function(item){
      item.addEventListener("mouseenter", function(e){
        ...
        if(label) label.textContent = title;
        tx = Math.min(window.innerWidth - 230, e.clientX + 90);
        ty = e.clientY;
  ...
  function init(){
    ...
    // Espera o load interno do canvas do Nimo e cria só um traço suave.
    setTimeout(initialSoftSweep, 1200);

    window.addEventListener("keydown", function(e){
      if((e.key || "").toLowerCase() === "s"){
        document.body.classList.toggle("v33-smoke-strong");
      }
    });

    console.info("[V33] Balanced Nimo global smoke + DreamLab preview OK");
  }

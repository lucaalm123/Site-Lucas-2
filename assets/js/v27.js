...
(function(){
  ...
  function bridgeFluidCanvas(){
    ...
    const feed = (type, e) => {
      ...
      if(type === "mousemove" && now - last < 9) return;
      last = now;
  ...
  document.addEventListener("DOMContentLoaded", ()=>{
    ...
    console.info("[V27] Visible aurora boost + saturated fluid OK");
  });
})();
...
(function(){
  ...
  let x = targetX;
  let y = targetY;
  let trailX = window.innerWidth * .55;
  let trailY = window.innerHeight * .48;

  function ensureAuroraLayer(){
    if(document.getElementById("v27-aurora-boost")) return;
    const layer = document.createElement("div");
    layer.id = "v27-aurora-boost";
    layer.setAttribute("aria-hidden", "true");
    document.body.prepend(layer);
  }

  function setVars(){
    document.documentElement.style.setProperty("--mouse-x", x.toFixed(1) + "px");
    document.documentElement.style.setProperty("--mouse-y", y.toFixed(1) + "px");
    document.documentElement.style.setProperty("--trail-x", trailX.toFixed(1) + "px");
    document.documentElement.style.setProperty("--trail-y", trailY.toFixed(1) + "px");
  }

  function animate(){
    x += (targetX - x) * 0.105;
    y += (targetY - y) * 0.105;
    trailX += (x - trailX) * 0.038;
    trailY += (y - trailY) * 0.038;
    setVars();
    requestAnimationFrame(animate);
  }

  function initV27AuroraBoost(){
    ensureAuroraLayer();
    ...
    // Pressionar A alterna modo ultra para comparar intensidade.
    window.addEventListener("keydown", function(e){
      if((e.key || "").toLowerCase() === "a"){
        document.body.classList.toggle("v27-ultra-fluid");
      }
    });

    console.info("[V27] Visible aurora boost OK");
  }

  if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", initV27AuroraBoost);
  } else {
    initV27AuroraBoost();
  }
})();

...
(function(){
  ...
  document.addEventListener("DOMContentLoaded", ()=>{
    ...
    console.info("[V29] Black background + visible real Nimo smoke OK");
  });
})();



/* V29 — Real Nimo smoke visibility helper.
   Não cria overlay. Apenas alimenta o canvas real do Nimo para o efeito aparecer. */
(function(){
  "use strict";
  ...
  function dispatchToCanvas(type, x, y, buttons){
  ...
  function bridgeRealMouse(){
    let last = 0;
    window.addEventListener("pointermove", function(e){
      const now = performance.now();
      if(now - last < 8) return;
      last = now;
      dispatchToCanvas("mousemove", e.clientX, e.clientY, e.buttons || 1);
    }, { passive: true });

    window.addEventListener("pointerdown", function(e){
      dispatchToCanvas("mousedown", e.clientX, e.clientY, 1);
    }, { passive: true });

    window.addEventListener("pointerup", function(e){
      dispatchToCanvas("mouseup", e.clientX, e.clientY, 0);
    }, { passive: true });
  }

  function primeSmoke(){
    // Pequeno rastro inicial do próprio Nimo para o usuário ver que está ativo.
    ...
    dispatchToCanvas("mousedown", start, y, 1);

    for(let i = 0; i <= steps; i++){
      setTimeout(function(){
        const t = i / steps;
        const x = start + (end - start) * t;
        const yy = y + Math.sin(t * Math.PI * 2) * 42;
        dispatchToCanvas("mousemove", x, yy, 1);
        if(i === steps) dispatchToCanvas("mouseup", x, yy, 0);
      }, 220 + i * 18);
    }
  }
  ...
  function init(){
    bridgeRealMouse();
    setTimeout(primeSmoke, 650);

    window.addEventListener("keydown", function(e){
      const key = (e.key || "").toLowerCase();
      if(key === "f"){
        document.body.classList.toggle("v29-smoke-strong");
      }
    });

    console.info("[V29] Real Nimo smoke helper OK");
  }

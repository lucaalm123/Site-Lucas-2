(function () {
  ...
  function initScrollFillText() {
    $$(".scroll-fill-text").forEach(function (el) {
      ...
      el.innerHTML = text.split(" ").map(function (word, index) {
        return '<span class="scroll-fill-word" style="--word-progress:0" data-word-index="' + index + '">' + escapeHtml(word) + '</span>';
      }).join(" ");
  ...
  function updateScrollFillText() {
    $$(".scroll-fill-text").forEach(function (el) {
      ...
      words.forEach(function (word, index) {
        var local = clamp((progress * (count + 2) - index) / 2.2, 0, 1);
  ...
  function updateStackedCards() {
    ...
    cards.forEach(function (card, index) {
      ...
      var scale = clamp(1 - abs * 0.075, 0.78, 1);
      var opacity = clamp(1 - abs * 0.24, 0.32, 1);
  ...
  function updateTimelineProgress() {
    ...
    var viewport = window.innerHeight || document.documentElement.clientHeight;
    var progress = clamp((viewport * 0.72 - rect.top) / Math.max(1, rect.height - viewport * 0.25), 0, 1);
  ...
  function initMagneticButtons() {
    ...
    $$(".magnetic-button").forEach(function (button) {
      button.addEventListener("mousemove", function (event) {
        ...
        button.style.setProperty("--shift-x", (x * 0.10).toFixed(2) + "px");
        button.style.setProperty("--shift-y", (y * 0.16).toFixed(2) + "px");

        var icon = $(".nimo-cta-icon, .header-arrow-icon", button);
        if (icon) {
          icon.style.transform = "translate(" + (x * 0.12).toFixed(2) + "px," + (y * 0.12).toFixed(2) + "px)";
        }
      });
  ...
  function initPortfolioHover() {
    // Hover is mostly CSS-driven; this function exists to keep motion modules explicit.
  ...
  function initParticleCanvas() {
    ...
    function resize() {
      ...
      particles = Array.from({ length: window.innerWidth < 720 ? 42 : 95 }, function (_, i) {
        return {
          ...
          a: 0.18 + Math.random() * 0.70,
          hue: i % 5 === 0 ? "255,46,117" : (i % 7 === 0 ? "53,195,255" : "217,255,47")
        };
      });
    }
    ...
    function draw() {
      ...
      particles.forEach(function (p) {
        ...
        if (dist < 120) {
          p.x += dx / Math.max(dist, 1) * 0.25;
          p.y += dy / Math.max(dist, 1) * 0.25;
        }
